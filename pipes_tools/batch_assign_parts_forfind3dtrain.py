...
DEFAULT_OBJECTS_ROOT = Path(
    "/x2robot_v2/lanejin/new_data/cosmo3d/dataset/datasets/datasets/train/3dcompat/forfind3dtrain"
)
...
# Confirmed point-cloud rotation matrix.
...
def setup_logger(log_path: Path) -> logging.Logger:
    logger = logging.getLogger("batch_assign_parts")
...
def parse_uid_from_dirname(name: str) -> str | None:
    # e.g. coarse_b'00_00a' -> 00_00a
    m = re.search(r"'([^']+)'", name)
    if m:
        return m.group(1)
    # fallback: if no quotes, try after coarse_b
...
def process_one_item(
    ...
    colored = colorize_points(points_raw, labels, len(part_names))
    colored.export(ply_out_path)

    uniq, cnt = np.unique(labels, return_counts=True)
    stat_path = item_dir / "parts_summary.txt"
    lines = ["part_index\tpart_name\tcount"]
    ...
    logger.info(
        "[done] uid=%s points=%d parts=%d saved=%s,%s",
        uid,
        points_raw.shape[0],
        len(part_names),
        parts_out_path.name,
...
def iter_object_dirs(objects_root: Path) -> List[Path]:
    return sorted([p for p in objects_root.iterdir() if p.is_dir()])


def main() -> None:
    parser = argparse.ArgumentParser(description="Batch assign part labels for forfind3dtrain points.")
    ...
    log_path = script_dir / "batch_assign_parts_forfind3dtrain.log"
    ...
    for item_dir in obj_dirs:
        try:
            ...
            if success and msg == "processed":
                ...
                skip += 1
                logger.info("[skip] %s", item_dir.name)
            else:
                fail += 1
                logger.warning("[skip/fail] %s | %s", item_dir.name, msg)
        except Exception as exc:
            fail += 1
            logger.exception("[error] %s | %s", item_dir.name, exc)
