...
class TrainingData(Dataset):
    def __init__(self, data_root, parts_suffix=""):
        ...
        self.parts_suffix = parts_suffix  # e.g. "_partsam" for PartSAM-generated labels
        # with open(f"{data_root}/split/train.txt", "r") as f:
        #     self.obj_path_list = f.read().splitlines()
        with open(f"{data_root}/train.txt", "r") as f:
            self.obj_path_list = f.read().splitlines()

        # Filter: only keep samples that have the part label files
        if parts_suffix:
            filtered = []
            skipped = 0
            for p in self.obj_path_list:
                if os.path.exists(f"{p}/mask2points{parts_suffix}.pt"):
                    filtered.append(p)
                else:
                    skipped += 1
            if skipped:
                print(f"TrainingData: {skipped}/{len(self.obj_path_list)} samples missing {parts_suffix} files, using {len(filtered)}")
    ...
    def __getitem__(self, item):
        ...
        with open(f"{data_dir}/mask_labels{self.parts_suffix}.txt", "r") as f:
            labels = f.read().splitlines()
        mask_pts = torch.load(f"{data_dir}/mask2points{self.parts_suffix}.pt", weights_only=True).cpu()  # 强制CPU
