...
import os
import sys
# The project root MUST stay at the FRONT of sys.path. UTONIA_ROOT contains a
# *second* package literally named `pipes` (the Utonia shim, exposing
# `pipes.exfeats.feature_pca_copule.init_utonia_model`). The online-Utonia
# runtime prepends UTONIA_ROOT to sys.path, so any process that inherits that
# sys.path (DataLoader workers, DDP spawn re-exec of this file) would otherwise
# resolve `import pipes` to the Utonia shim and fail on `pipes.ours_token3d_acc`.
# Forcing the project root to index 0 here guarantees the project's `pipes` wins.
...
from pipes.ours_token3d_acc.data import (
    TrainingData,
    EvalData,
    collate_fn,
    ensure_utonia_local_ckpt,
    resolve_utonia_device,
)
...
def init_utonia_runtime(rank, enable_online_utonia: bool):
    if not enable_online_utonia:
        return None, None
    ...
    # UTONIA_ROOT contains its own `pipes/` package; the shim (and the line above)
    # may have prepended UTONIA_ROOT to sys.path, which would shadow this project's
    # `pipes` package in any subprocess that inherits sys.path (e.g. DataLoader
    # workers). `utonia` is already imported and cached, so demote UTONIA_ROOT to
    # the lowest priority and re-assert the project root at the front.
    while utonia_root in sys.path:
        sys.path.remove(utonia_root)
    sys.path.append(utonia_root)
    if _project_root in sys.path:
        sys.path.remove(_project_root)
    sys.path.insert(0, _project_root)

    ensure_utonia_local_ckpt()
    requested_device = f"cuda:{rank}" if torch.cuda.is_available() else resolve_utonia_device()
    model, device = init_utonia_model(device=requested_device)
    model.eval()
    return model, device


def attach_utonia_feature(data, utonia_model, utonia_device):
    if utonia_model is None:
        return

    for required_key in ("coord", "grid_coord", "offset", "feat"):
        if required_key not in data:
            raise KeyError(f"Missing key '{required_key}' for online utonia inference")
    if data["feat"].ndim != 2 or int(data["feat"].shape[1]) != 6:
        raise RuntimeError(f"Expected data['feat'] with shape [N, 6], got {tuple(data['feat'].shape)}")

    # Utonia expects 9D feature = coord(3) + feat(6).
    utonia_input = {
        "coord": data["coord"].to(utonia_device, non_blocking=True),
        "grid_coord": data["grid_coord"].to(utonia_device, non_blocking=True),
        "offset": data["offset"].to(utonia_device, non_blocking=True),
        "feat": torch.cat([data["coord"], data["feat"]], dim=1).to(utonia_device, non_blocking=True),
    }

    with torch.inference_mode():
        utonia_out = utonia_model(utonia_input)
        for _ in range(2):
    ...
    data["utonia_feat"] = utonia_out.feat.to(data["coord"].device, non_blocking=True)
...
def create_data_loader(rank, data_root, shuffle_train, shuffle_test,
    ...
    train_data = TrainingData(data_root, parts_suffix=parts_suffix)
...
def train(rank, world_size, args):
    ...
    utonia_model, utonia_device = init_utonia_runtime(
        rank=rank,
        enable_online_utonia=args.enable_online_utonia,
    )
    if rank == 0:
        print(
            "Utonia runtime: "
            + ("enabled on " + str(utonia_device) if utonia_model is not None else "disabled")
        )
...
def main():
    ...
    parser.add_argument('--parts_suffix', default='', type=str, help="Suffix for part label files (e.g. '_partsam')")
    ...
    parser.add_argument('--enable_online_utonia', type=str2bool, default=True,
                        help="Compute utonia features in training step instead of DataLoader workers")
    ...
    print(f"Online Utonia in train step: {args.enable_online_utonia}")
