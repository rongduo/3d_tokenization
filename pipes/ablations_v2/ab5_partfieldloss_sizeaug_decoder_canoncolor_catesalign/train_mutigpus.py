...
from release_pipeline5ab.ab5_partfieldloss_sizeaug_decoder_canoncolor_catesalign.data import TrainingData, EvalData, collate_fn
from release_pipeline5ab.ab5_partfieldloss_sizeaug_decoder_canoncolor_catesalign.mixdecodernet import PointSemSegWithDecoder
from release_pipeline5ab.ab5_partfieldloss_sizeaug_decoder_canoncolor_catesalign.mixdecoderloss import BalancedMaskCrossEntropyLoss,CanonicalColorLoss
...
def train(rank, world_size, args):
    ...
    for epoch in range(start_epoch, args.n_epoch + 1):
        ...
        for iter_idx, data in enumerate(train_iterator):
            ...
            backbone_feat, decoder_out, canoncolor_out, decoder_offset = model(data)

            # # debug 查看gt caonoical color 和 预测的ccolor的维度
            # print('data[offset] ： ', data['offset'].shape)
            # print('data[offset] ： ', data['offset'])  # 和color_offset是一样的
            # print('pre:', canoncolor_out.shape)
            # print('canoncial_color:', data['canoncial_color'].shape)
            # asdf
            ...
            # print('decoder_loss:', decoder_loss.item())
            # print('canoncolor_loss:', canoncolor_loss.item())
            # asdf

            # 计算总损失
            total_loss = baseline_loss + weighted_decoder_loss + weighted_canoncolor_loss
