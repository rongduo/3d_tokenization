...
from pipes.ours_canon_align_bbox.data import TrainingData, EvalData, collate_fn
from pipes.ours_canon_align_bbox.mixdecoderloss import BalancedMaskCrossEntropyLoss,CanonicalColorLoss
from pipes.ours_canon_align_bbox.mixdecoderloss_bboxoff import PointBasedBBoxOffsetLoss
...
def train(rank, world_size, args):
    ...
    # 最后包装DDP（关键修改：find_unused_parameters=True，否则未参与回传会直接报错）
    if world_size > 1:
        model = torch.nn.parallel.DistributedDataParallel(
            model,
            device_ids=[rank],
            find_unused_parameters=True,  # 必须设为True，才能检测未参与回传的参数
        )
...
def main():
    ...
    # 新增：梯度检查开关（默认开启）
    # =========================================================================
    parser.add_argument('--check_grad_flow', type=bool, default=True, 
                        help="Whether to check unused parameters (no gradient flow), default=True")
