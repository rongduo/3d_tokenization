...
from model.training.loss import DistillLossContrastive , FieldDistillLossContrastive # , HardDistillLossContrastive

from transformers import AutoTokenizer, AutoModel
import random
from model.evaluation.core import visualize_3d_upsample

from release_pipeline5ab.ab2_partfieldloss_sizeaug.data import TrainingData, EvalData, collate_fn
...
def train(rank, world_size, args):
    ...
    criterion = FieldDistillLossContrastive()
    ...
    for epoch in range(args.n_epoch):
        ...
        for iter_idx, data in enumerate(train_iterator):
            ...
            try:  
                ...
                '''# 关键调试：检查第0号参数的梯度（第二次迭代时重点观察）
                if iter_idx == 1 and rank == 0:  # 第二次迭代（iter=1）时检查
                    print("\n===== 第二次迭代参数梯度检查 =====")
                    # 取模型参数（DDP包装时需用model.module）
                    params = model.module.parameters() if world_size > 1 else model.parameters()
                    for idx, (param, (info_idx, info_name)) in enumerate(zip(params, param_info)):
                        if idx == 0:  # 重点检查第0号参数
                            has_grad = param.grad is not None and not torch.allclose(param.grad, torch.zeros_like(param.grad))
                            print(f"第 {info_idx} 号参数 ({info_name}) 的梯度存在: {has_grad}")
                            if not has_grad:
                                print(f"警告：第 {info_idx} 号参数 ({info_name}) 仍无有效梯度！")'''
