...
from release_module.decoder.ptstextnet import PointCloudTextBboxTransformer, PointCloudTextBboxTransformerConfig

class PointSemSegWithbboxDecoder(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        # 2. 初始化你的decoder
        cfg = PointCloudTextBboxTransformerConfig()
        cfg_dict = asdict(cfg)
        self.decoder = PointCloudTextBboxTransformer(cfg_dict)
        # 3. 保留原模型中的温度参数（如果需要）
    ...
    def forward(self, data):
        ...
        # 2. decoder处理特征
        decoder_out, decoder_offset, bbox_pred, bbox_offset = self.decoder(data['coord'], backbone_feat,
        ...
        asdf'''
        return backbone_feat, decoder_out, decoder_offset, bbox_pred, bbox_offset  # 返回中间特征和最终输出
