...
import numpy as np
from pipes_eval.partnete.core import compute_3d_iou_upsample, compute_3d_iou_upsample_pro
import time
from pipes_eval.partnete.utils import set_seed, load_model
...
'''def evaluate3d(model, dataloader, panoptic=False, N_CHUNKS=1, visualize_seg=False, visualize_all_heatmap=False, savedir=None): # evaluate loader can only have batch size=1
    ...
    return full_miou
'''
def eval_category_partnete(data_root, category, model, apply_rotation=False, subset=False, decorated=True, visualize_seg=False, visualize_all_heatmap=False, c=None, savedir=None, testname=None):
...
def eval_partnete(data_root, model, apply_rotation, subset, decorated, cfv=False, save_path=None, testname=None):
