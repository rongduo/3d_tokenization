...
def prep_points_train(xyz, rgb, normal, mask2pt):
    ...
    data_dict = RandomScale(scale=[0.9, 1.1], anisotropic=True)(data_dict)
...
class EvalData3D(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # print('pts_xyz:', pts_xyz.shape, 'normal:', normal.shape, 'pts_rgb:', pts_rgb.shape, 'gt:', gt.shape)
...
class EvalPartNetE(Dataset):
    def __init__(self, data_root, category, apply_rotation=False, subset=False, decorated=True):
        ...
        # self.part_names = ['lid', 'body', 'bottom']  # 给多标签进行测试
    ...
    def __getitem__(self, item):
        ...
        if self.apply_rotation:
            rots = torch.load(f"model/evaluation/benchmark/benchmark_reproducibility/partnete/rand_rotation.pt")  # 测试的时候，并不进行旋转的扰动
            rot = rots[item]
        else:
            rot = torch.eye(3)
            # print('11111')
        ...
        pts_xyz = torch.tensor(np.asarray(pcd.points)).float()
        '''# 旋转到和3dcompat一致会怎样？
        def rotate_around_x(xyz, angle_deg=90):
            """绕X轴旋转指定角度"""
            ...
            rotated_xyz = xyz_np @ rotation_matrix.T
            return torch.tensor(rotated_xyz, dtype=xyz.dtype).to(xyz.device) if isinstance(xyz, torch.Tensor) else rotated_xyz
        def rotate_around_y(xyz, angle_deg=180):
            """绕Y轴旋转指定角度"""
            angle_rad = np.radians(angle_deg)
            rotation_matrix = np.array([
            ...
            xyz_np = xyz.cpu().numpy() if isinstance(xyz, torch.Tensor) else xyz
            rotated_xyz = xyz_np @ rotation_matrix.T
            # print('success rotate around y')
            return torch.tensor(rotated_xyz, dtype=xyz.dtype).to(xyz.device) if isinstance(xyz, torch.Tensor) else rotated_xyz
        rotated_x = rotate_around_x(pts_xyz, angle_deg=90)
        pts_xyz = rotate_around_y(rotated_x, angle_deg=180)'''
        ...
        '''
        ########debug
        object_path = f"{file_path}/pc.ply"
        ...
        # pts_xyz *= (0.75 / float(scale)) # put in 1.5-size box
        random_indices = torch.randint(0, pts_xyz.shape[0], (5000,))
        pts_xyz_subsampled = pts_xyz[random_indices].float()
        pts_rgb_subsampled = pts_rgb[random_indices].float()
        normal_subsampled = normal[random_indices].float()
        gt = torch.tensor(np.load(f"{file_path}/label.npy",allow_pickle=True).item()['semantic_seg'])+1 # we make it agree with objaverse, 0 is unlabeled and 1-k labeled
        gt_subsampled = gt[random_indices]
        '''
...
class EvalShapeNetPart(Dataset):
    def __init__(self, data_path, class_choice, apply_rotation=False, subset=False, decorated=True, use_tuned_prompt=False):
        ...
        file = glob.glob(os.path.join(data_path, '*test*.h5'))
