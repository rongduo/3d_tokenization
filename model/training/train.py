...
def train(args):
    ...
    for epoch in tqdm(range(args.n_epoch), desc="Training epochs"):
        ...
        for data in (tqdm(train_loader, desc=f"Training epoch: {epoch}/{args.n_epoch}")):
            ...
            opt.step()
            scheduler.step()
            print(f"iter {iter} loss {loss.item()} lr {cur_lr} temperature {np.exp(model.ln_logit_scale.item())}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrained_path', type=str, default=None, help="Path to pretrained model") # 加载已经有得训练模型
