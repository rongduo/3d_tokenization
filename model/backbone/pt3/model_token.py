...
class PointTransformerV3(PointModule):
    def __init__(
        ...
        enc_channels=(128, 192, 256, 384, 512),
        ...
        self.utonia_feat_dim = utonia_feat_dim
        ...
        # 融合每点的颜色/法向 embedding 特征、token 特征和 utonia 特征后，压回 encoder 输入维度。
        fusion_in_dim = enc_channels[0] + self.token_feat_dim + self.utonia_feat_dim
        fusion_hidden_dim = max(enc_channels[0] * 4, 128)
        self.pre_enc_fusion = nn.Sequential(
            nn.Linear(fusion_in_dim, fusion_hidden_dim),
            nn.GELU(),
            nn.Linear(fusion_hidden_dim, enc_channels[0]),
        )
    ...
    def forward(self, data_dict):
        ...
        # 在 encoder 前做特征融合：
        # embedding 后特征(颜色/法向相关) + token_feat + utonia_feat -> 线性降维回 enc_channels[0]
        n_pts = point.feat.shape[0]
        token_feat = data_dict.get("token_feat", None)
        if not isinstance(token_feat, torch.Tensor) or token_feat.ndim != 2 or token_feat.shape[0] != n_pts:
            token_feat = point.feat.new_zeros((n_pts, self.token_feat_dim))
        else:
            token_feat = token_feat.to(device=point.feat.device, dtype=point.feat.dtype)
            if token_feat.shape[1] != self.token_feat_dim:
                token_feat = point.feat.new_zeros((n_pts, self.token_feat_dim))

        utonia_feat = data_dict.get("utonia_feat", None)
        if not isinstance(utonia_feat, torch.Tensor) or utonia_feat.ndim != 2 or utonia_feat.shape[0] != n_pts:
            utonia_feat = point.feat.new_zeros((n_pts, self.utonia_feat_dim))
        else:
            utonia_feat = utonia_feat.to(device=point.feat.device, dtype=point.feat.dtype)
            if utonia_feat.shape[1] != self.utonia_feat_dim:
                utonia_feat = point.feat.new_zeros((n_pts, self.utonia_feat_dim))

        fused_feat = torch.cat([point.feat, token_feat, utonia_feat], dim=1)
        point.feat = self.pre_enc_fusion(fused_feat)
...
class PointSemSeg(nn.Module):
    def __init__(self, args, dim_output, emb=128, init_logit_scale=np.log(1 / 0.07)):
        super().__init__()
